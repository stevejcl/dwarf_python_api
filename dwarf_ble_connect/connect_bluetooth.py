...
def update_htmlfile(ble_psd, ble_STA_ssid, ble_STA_pwd):
  ...
  if  modify1 and modify2 and modify3:
    log.info("The Html file to connect to Bluetooth has been updated accordingly")
    return True
  
  if modify1 is not True:
    log.warning("BluetoothPWD has not been updated")

  if modify2 is not True:
    log.warning("BleSTASSIDDwarf has not been updated")

  if modify3 is not True:
    log.warning("BleSTAPWDDwarf has not been updated")
